...
//사용자별  검색등록  API
exports.postSearchs = async function (params){
    const connection = await pool.getConnection(async (conn) => conn);
    const result = await Dao.postSearchd(connection, params);
    connection.release();
    return response(baseResponse.SUCCESS);
}

//특정검색기록 삭제 API
exports.deleteSearchlistbyIds = async function (searchId){
    const connection = await pool.getConnection(async (conn) => conn);
    const result = await Dao.deleteSearchlistbyIdd(connection, searchId);
    connection.release();
    return response(baseResponse.SUCCESS);
}

//사용자별 전체검색기록 삭제API
exports.deleteSearchlistbyUserIds = async function (searchId){
    const connection = await pool.getConnection(async (conn) => conn);
    const result = await Dao.deleteSearchlistbyUserIdd(connection, searchId);
    connection.release();
    return response(baseResponse.SUCCESS);
}
